...
const ShoppingListPage = () => {
  const [shoppingLists, setShoppingLists] = useState([
    { id: 1, name: 'Groceries', items: [{ id: 1, name: 'Apple', quantity: 2 }] },
    { id: 2, name: 'Stationery', items: [{ id: 1, name: 'Notebook', quantity: 5 }] }
  ]);
  ...
  const addItem = (listId, itemName) => {
    ...
    const updatedLists = shoppingLists.map((list) =>
      list.id === listId
        ? { ...list, items: [...list.items, { id: Date.now(), name: itemName, quantity: 1 }] }
        : list
    );
    setShoppingLists(updatedLists);
  };
